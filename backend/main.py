...
async def load_models():
    global yolo_model, cnn_model, cnn_transform, id2label
...
async def get_most_confident_detection(image_bytes):
    try:
        ...
        # Inferencia con YOLOv8, con confianza 0.01 (más permisivo)
        results = yolo_model(img_cv, conf=0.01)
        detections = results[0].boxes  # Boxes detectadas
...
async def predict_product(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Devolver solo el ID del producto
